...
impl Flags {
    ...
    /// - `follow links`
    pub fn new() -> Flags {
        Flags::from_int(libc::O_RDONLY | libc::O_CLOEXEC)
    }

    /// Returns if the `readable` flags is set.
    pub fn is_readable(&self) -> bool {
        **self & libc::O_ACCMODE == libc::O_RDWR ||
            **self & libc::O_ACCMODE == libc::O_RDONLY
    }
    ...
    pub fn set_readable(&mut self, val: bool) {
        **self = (**self & !libc::O_ACCMODE) | match (val, self.is_writable()) {
    ...
    pub fn set_writable(&mut self, val: bool) {
        **self = (**self & !libc::O_ACCMODE) | match (val, self.is_readable()) {
            (true,  true)  => libc::O_RDWR,
            (true,  false) => libc::O_WRONLY,
            (false, true)  => libc::O_RDONLY,
            (false, false) => 0,
        };
    }
    ...
    pub fn is_bypass_buffer(&self) -> bool { **self & libc::O_DIRECT != 0 }

    /// Enables or disables the `bypass buffer` flag.
    pub fn set_bypass_buffer(&mut self, val: bool) { self.set_bit(libc::O_DIRECT, val); }

    /// Checks if the `only directory` flag is set.
    ///
    /// If this flag is set, opening fails if the path does not refer to a directory.
    pub fn is_only_directory(&self) -> bool { **self & libc::O_DIRECTORY != 0 }

    /// Enables or disables the `only directory` flag.
    pub fn set_only_directory(&mut self, val: bool) { self.set_bit(libc::O_DIRECTORY, val); }

    /// Checks if the `exclusive` flag is set.
    ///
    /// If this flag and the `create` flag is set and the file already exists, opening the
    /// file fails.
    pub fn is_exclusive(&self) -> bool { **self & libc::O_EXCL != 0 }

    /// Enables or disables the `exclusive` flag.
    pub fn set_exclusive(&mut self, val: bool) { self.set_bit(libc::O_EXCL, val); }
    ...
    pub fn is_access_time_update(&self) -> bool { **self & libc::O_NOATIME == 0 }

    /// Enables or disables the `access time update` flag.
    pub fn set_access_time_update(&mut self, val: bool) { self.set_bit(libc::O_NOATIME, !val); }
    ...
    pub fn is_controlling_term(&self) -> bool { **self & libc::O_NOCTTY == 0 }

    /// Enables or disables the `controlling term` flag.
    pub fn set_controlling_term(&mut self, val: bool) { self.set_bit(libc::O_NOCTTY, !val); }

    /// Checks if the `follow links` flag is set.
    ///
    /// If this flag is not set and the path refers to a symlink, the symlink will be
    /// opened.
    pub fn is_follow_links(&self) -> bool { **self & libc::O_NOFOLLOW == 0 }

    /// Enables or disables the `follow links` flag.
    pub fn set_follow_links(&mut self, val: bool) { self.set_bit(libc::O_NOFOLLOW, !val); }
    ...
    pub fn is_truncate(&self) -> bool { **self & libc::O_TRUNC != 0 }

    /// Enables or disables the `truncate` flag.
    pub fn set_truncate(&mut self, val: bool) { self.set_bit(libc::O_TRUNC, val); }

    /// Checks if the `append` flag is set.
    ///
    /// If this flag is set, all writes will append to the end of the file.
    pub fn is_append(&self) -> bool { **self & libc::O_APPEND != 0 }

    /// Enables or disables the `append` flag.
    pub fn set_append(&mut self, val: bool) { self.set_bit(libc::O_APPEND, val); }
    ...
    pub fn is_signal_io(&self) -> bool { **self & libc::O_ASYNC != 0 }

    /// Enables or disables the `signal io` flag.
    pub fn set_signal_io(&mut self, val: bool) { self.set_bit(libc::O_ASYNC, val); }
    ...
    pub fn is_data_synchronized(&self) -> bool { **self & libc::O_DSYNC != 0 }

    /// Enables or disables the `data synchronized` flag.
    pub fn set_data_synchronized(&mut self, val: bool) { self.set_bit(libc::O_DSYNC, val); }

    /// Checks if the `non blocking` flag is set.
    ///
    /// If this flag is set and reading or writing would block, an error will be returned
    /// instead.
    pub fn is_non_blocking(&self) -> bool { **self & libc::O_NONBLOCK != 0 }

    /// Enables or disables the `non blocking` flag.
    pub fn set_non_blocking(&mut self, val: bool) { self.set_bit(libc::O_NONBLOCK, val); }

    /// Checks if the `synchronized` flag is set.
    ///
    /// If this flag is set and writing succeeds, then all data has been transferred to
    /// the disk hardware.
    pub fn is_synchronized(&self) -> bool { **self & libc::O_SYNC != 0 }

    /// Enables or disables the `synchronized` flag.
    pub fn set_synchronized(&mut self, val: bool) { self.set_bit(libc::O_SYNC, val); }
    ...
    pub fn is_path_fd(&self) -> bool { **self & libc::O_PATH != 0 }

    /// Enables or disables the `path fd` flag.
    pub fn set_path_fd(&mut self, val: bool) { self.set_bit(libc::O_PATH, val); }

    /// Checks if the `temp file` flag is set.
    pub fn is_temp_file(&self) -> bool { **self & libc::O_TMPFILE != 0 }
    ...
    pub fn set_temp_file(&mut self, val: bool) { self.set_bit(libc::O_TMPFILE, val); }
