use result::{Result};
use libc::{uid_t, gid_t, getpwuid_r, c_char, size_t, passwd, getpwnam_r, c_int};
use conf::{self};
...
use errno::{self};
use rust::{AsStr, AsLinuxStr, ByteSliceExt};
use fs::{File};

/// Constant default value for non-allocating user info buffer size.
pub const INFO_BUF_SIZE: usize = 1024;

/// Retrieves a recommended buffer size from the operating system.
pub fn info_buf_size() -> usize {
    match conf::sys::GETPW_R_SIZE_MAX.get() {
...
pub trait UserInfo {
    ...
    fn password(&self) -> &[u8];
    /// User id of the user.
    fn user_id(&self)  -> uid_t;
    /// Group id of the user.
    fn group_id(&self) -> gid_t;
    /// Comment of the user.
    fn comment(&self)  -> &[u8];
...
impl<'a> UserInfo for Info<'a> {
    ...
    fn user_id(&self)  -> uid_t { self.user_id }
    fn group_id(&self) -> gid_t { self.group_id }
