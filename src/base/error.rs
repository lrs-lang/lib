...
create! {
    ...
    NoBlockSpecialFile      = ENOTBLK         , "Resource busy",
    ResourceBusy            = EBUSY           , "Block special file required",
    FileExists              = EEXIST          , "File exists",
    CrossFileSystemLink     = EXDEV           , "Attempted to link across file systems",
    ...
    ReadOnlyFileSystem      = EROFS           , "Operation not permitted on read-only file system",
